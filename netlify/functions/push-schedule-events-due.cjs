...
const { handler: sendEventsDueHandler } = require('./push-send-events-due.cjs');
...
exports.handler = async () => {
  try {
    const timeZone = String(process.env.PUSH_EVENTS_DUE_TIME_ZONE || 'Europe/Rome').trim() || 'Europe/Rome';
    const tomorrowHourLocal = parseHour(
      process.env.PUSH_EVENTS_DUE_HOUR_TOMORROW_LOCAL || process.env.PUSH_EVENTS_DUE_HOUR_LOCAL,
      20
    );
    const todayHourLocal = parseHour(process.env.PUSH_EVENTS_DUE_HOUR_TODAY_LOCAL, 10);
    const profileIds = parseProfileIds(process.env.PUSH_EVENTS_DUE_PROFILE_IDS);
    ...
    if (windows.length === 0) {
      return sendJson(200, {
        ...
        scheduled: true,
        skipped: true,
        reason: `Ora locale ${localHour} fuori dalle finestre eventi (${todayHourLocal}, ${tomorrowHourLocal}).`,
        timeZone,
        runDate,
        localHour,
        hours: {
